...
export const createReview = async (req, res) => {
  try {
    const { product_id, user_id, quality, service, value, comment, approved } = req.body;

    // Create a new review instance with data from the request body
    const newReview = new Review({
      product_id: mongoose.Types.ObjectId(product_id), // Ensure ObjectId type
      user_id: mongoose.Types.ObjectId(user_id),       // Ensure ObjectId type
      quality,
      service,
      value,
      comment,
